...
var dataUser
$.ajax({
  url: "/annuaire/organigrammeDataWithService",
  type: "POST",
  async: false,
  data:
  {
    'service' : document.getElementById("serviceToGet").innerText,
  },
  success: function (data) 
  {
    if(data.code == -1)
    {
      dataUser = "";
      createAlert(
        "Chef(fe) de service introuvable. Raison possible : Fonction Chef(fe) de service non désignée sur une personne du service",
        "danger",false,10000,"alertsDisplay");
    }
    else
    {
      dataUser = data;
