...
// Le dernier segment contient probablement votre paramètre
var parametre = segments[segments.length - 1];
var dataUser;
$.ajax({
  url: "/annuaire/organigrammeEquipe",
  type: "POST",
  async: false,
  data:{
    'nniSup' : parametre
    ...
    <div style="padding-top:30px;background-color:none;margin-left:1px;height:${
      ...
      <img src=" ${
        ...
        <div style="display:flex;justify-content:space-between;padding-left:15px;padding-right:15px;">
          ...
          if(d.data.connect != null)
          {
            res += `<div > Connect : ${d.data.connect}</div>`;
          }
          res += `        </div>
          </div>     
    </div>`;
...
function viewProfil(nni, imgUrl)
{
  $.ajax({
    ...
    success: function (data) 
    {
      ...
      document.getElementById("btnOrganigramme").addEventListener("click", function(event) {
        event.preventDefault(); // Pour empêcher le lien de s'ouvrir immédiatement
    
        var href = this.getAttribute("href"); // Récupérer l'URL du lien
        window.open(href, "_self"); // Ouvrir l'URL dans la même fenêtre
    });
